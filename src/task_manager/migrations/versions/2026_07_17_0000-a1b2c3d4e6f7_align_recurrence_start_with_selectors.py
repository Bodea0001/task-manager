"""align recurrence start with calendar selectors
...
def _date_generator_sql(*, force_anchor_occurrence: bool) -> str:
    weekly_sequence = (
        "weekly_periods.period_no * ranked_weekdays.weekday_count "
        "+ ranked_weekdays.weekday_rank "
        "- ranked_weekdays.weekdays_through_anchor + 1"
        if force_anchor_occurrence
        else "weekly_periods.period_no * ranked_weekdays.weekday_count "
        "+ ranked_weekdays.weekday_rank "
        "- ranked_weekdays.weekdays_before_anchor"
    )
    weekly_source = (
        """
    ...
    monthly_lower_bound = (
        "nominal_date >= p_anchor_date AND planned_date >= p_anchor_date"
        if force_anchor_occurrence
        else "planned_date >= p_anchor_date"
    )

    return f"""
        CREATE OR REPLACE FUNCTION generate_task_recurrence_dates(
            p_frequency text,
            p_step integer,
            p_anchor_date date,
            p_weekdays smallint[],
        ...
        AS $function$
            ...
            ranked_weekdays AS (
                SELECT
                    ...
                    ) OVER ()::integer AS weekdays_before_anchor,
                    count(*) FILTER (
                        WHERE weekday <= extract(isodow FROM p_anchor_date)::integer
            ...
            weekly_candidates AS (
                SELECT
                    ({weekly_sequence})::integer AS sequence_no,
                    (
                        bounds.anchor_week
                        + (
...
def upgrade() -> None:
    """Treat the anchor as the inclusive start of calendar-based recurrence."""
    op.execute(_date_generator_sql(force_anchor_occurrence=False))
    op.alter_column(
        "task_recurrence_series",
        "anchor_date",
        existing_type=sa.Date(),
        existing_nullable=False,
        comment="Включительная дата начала действия правила",
    )


def downgrade() -> None:
    """Restore the anchor as an unconditional first occurrence."""
    op.execute(_date_generator_sql(force_anchor_occurrence=True))
    op.alter_column(
        "task_recurrence_series",
        "anchor_date",
        existing_type=sa.Date(),
        existing_nullable=False,
        comment="Дата первого повторения",
    )
