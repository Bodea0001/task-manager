...
from agents.prompts import RECURRENCE_TEMPLATE_CREATION_AGENT_PROMPT
...
from agents.tools.tasks import create_task_recurrence_template
...
def create_recurrence_template_creation_agent(model: str | BaseChatModel) -> AgentGraph:
    """Create an agent focused on creating recurring-task templates."""
    return create_agent(
        model=model,
        tools=_TOOLS,
        system_prompt=RECURRENCE_TEMPLATE_CREATION_AGENT_PROMPT,
        state_schema=cast(Any, AgentState),
        context_schema=AgentContext,
        response_format=ToolStrategy(AgentResult),
        checkpointer=None,
        middleware=_MIDDLEWARES,
        name="recurrence_template_creation_agent",
    )
